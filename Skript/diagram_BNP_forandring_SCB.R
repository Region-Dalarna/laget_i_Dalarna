diagram_BNP_SCB <- function(output_mapp = "G:/Samhällsanalys/Statistik/Näringsliv/basfakta/",
  ...
  BNP_df <- pxweb2_hamta_data(
    tabell = "TAB4553",
    query = list(
      Anvandningstyp = "- BNP till marknadspris",
      ContentsCode = "Säsongrensad, volymförändring föregående period, procent",
      Tid = "*"
    )) %>% 
      rename("Säsongrensad, volymförändring föregående period, procent" = value) %>% 
