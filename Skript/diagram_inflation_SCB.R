diagram_inflation_SCB <- function(output_mapp = "G:/Samhällsanalys/Statistik/Näringsliv/basfakta/",
                                             spara_figur = TRUE, # Skall diagrammet sparas
                                             returnera_data = FALSE, # Skall data returneras
                                             returnera_figur = TRUE,
                                             valda_farger = diagramfarger("rus_sex"),
                                             antal_etiketter = 24, # Intervall mellan visade etiketter (i månader)
                                             startvarde = "1993-01"){# Startvärde för diagrammet.Finns från 1987-01
  
  # Skript som skapar ett linjediagram för inflation (enligt KPIF) från valt startvärde till sista år
  # Skapat av Jon Frank med hjälp av Peter Möllers automatiska skript, 2024-04-12

  if (!require("pacman")) install.packages("pacman")
  p_load(tidyverse)
  
  #source("https://raw.githubusercontent.com/Region-Dalarna/hamta_data/main/hamta_KPIF_tid_scb.R")
  #source("https://raw.githubusercontent.com/Region-Dalarna/hamta_data/refs/heads/main/hamta_KPIF_tid_scb_nytt_basar.R")
  ...
  # Tidigare pxweb
  # KPIF_df <- hamta_KPIF_tid_scb_nytt_basar(
  #   cont_klartext = "KPIF, 12-månadsförändring, 2020=100",			 #  Finns: "KPIF, index, 1987=100", "KPIF, månadsförändring, 1987=100", "KPIF, 12-månadsförändring, 1987=100"
  ...
  # Länk till tabell https://www.statistikdatabasen.scb.se/pxweb/sv/ssd/START__PR__PR0101__PR0101G/KPIF2020/
  
  KPIF_df <- pxweb2_hamta_data(
    tabell = "TAB6590",
    query = list(
      ContentsCode = "KPIF, 12-månadsförändring, 2020=100",
      Tid = "*"
    )) %>% 
    rename(`KPIF, 12-månadsförändring, 2020=100` = value) %>% 
  ...
  KPIF_df <- KPIF_df %>%   
    mutate(ar=substr(månad,1,4),
      ...
      rename("KPIF" = `KPIF, 12-månadsförändring, 2020=100`) %>% 
        mutate(variabel = "Inflation") %>% 
      filter(Period>=startvarde)
  
  if(returnera_data == TRUE){
    assign("KPI_df", KPIF_df, envir = .GlobalEnv)
  }
  ...
  gg_obj <- SkapaLinjeDiagram(skickad_df = KPI_utskrift %>% 
                                mutate(manad_ar = factor(manad_ar, levels = unique(manad_ar), ordered = TRUE)),
                                       ...
                                       x_axis_visa_var_xe_etikett = antal_etiketter,
