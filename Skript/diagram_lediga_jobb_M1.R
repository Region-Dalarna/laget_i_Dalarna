...
diagramtitel <- glue("Aggregerad matchning (M1){region_txt}")
...
gg_obj <- SkapaStapelDiagram(skickad_df = lediga_jobb_M1_df %>% 
                             ...
                             manual_color = if ("variabel" %in% names(lediga_jobb_M1_df) & length(unique(lediga_jobb_M1_df$variabel)) > 1) diagramfarger("rus_sex") else diagramfarger("rus_sex")[1],
