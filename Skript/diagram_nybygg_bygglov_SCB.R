diagram_nybyggnation_bygglov <- function(region_vekt = "20",
                                 ...
                                 hustyp_klartext = "småhus",			 #  NA = tas inte med i uttaget,  Finns: "flerbostadshus", "småhus"
  ...
  if(diag_nybyggnation==TRUE){
    ...
    if(hustyp_klartext == "*"){
      diagramtitel <- glue("Påbörjade bostäder i {unique(nybyggnation_df$region) %>% skapa_kortnamn_lan() %>% list_komma_och()}")}
    else{
      diagramtitel <- diagramtitel <- glue("Påbörjade {unique(nybyggnation_df$hustyp)} i {unique(nybyggnation_df$region) %>% skapa_kortnamn_lan() %>% list_komma_och()}")
    }
    
    #diagramtitel <- glue("Påbörjade {unique(nybyggnation_df$hustyp)} i {unique(nybyggnation_df$region) %>% skapa_kortnamn_lan() %>% list_komma_och()}")
    ...
    gg_obj <- SkapaLinjeDiagram(skickad_df = nybyggnation_df %>% 
                                  mutate(kvartal_ar = factor(kvartal_ar, levels = unique(kvartal_ar), ordered = TRUE)),
                                            ...
                                            skickad_y_var = "varde",
                                            skickad_x_grupp = ifelse(hustyp_klartext != "*",NA,"hustyp"),
  ...
  if(diag_bygglov == TRUE){
    ...
    if(hustyp_klartext == "*"){
      diagramtitel <- glue("Bygglov för bostäder i {unique(bygglov_df$region) %>% skapa_kortnamn_lan() %>% list_komma_och()}")}
    else{
      diagramtitel <- glue("Bygglov för {unique(nybyggnation_df$hustyp)} i {unique(bygglov_df$region) %>% skapa_kortnamn_lan() %>% list_komma_och()}")
    }
