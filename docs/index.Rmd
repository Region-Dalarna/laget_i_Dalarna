...
<ul>
<li> Hushållens och företagens syn på ekonomin har förbättrats på sistone och aktiviteten i ekonomin (BNP) ökade under tredje kvartalet 2024, men inflationen är något högre än Riksbankens inflationsmål. </li>
<li> Nybyggnation av bostäder har ökat i Dalarna på sistone. Dessutom ökade priserna på småhus något i `r priser_df %>% filter(Period==last(Period),region=="Dalarna") %>% select(manad_long)` `r priser_df %>% filter(Period==last(Period),region=="Dalarna") %>% select(ar)` jämfört med månaderna innan.</li>
<li> Antalet anställda berörda av konkurser i Dalarna minskade under en period mot slutet av 2024, jämfört med året innan, men ökade tydligt i januari 2025. </li>
<li> Arbetslösheten i Dalarna, som är klart under genomsnittet i Sverige, har under 2024, efter en lång period av nedgång, stabiliserats på ungefär samma nivå som föregående år.</li>
...
Genom att titta på förändring i BNP kan man få en uppfattning om hur aktiviteten i den svenska ekonomin förändras över tid. Eftersom Sveriges ekonomi, precis som de flesta andra länders, växer i normala fall, ökar BNP under en majoritet av kvartalen. Vid större kriser i samhället kan dock BNP minska avsevärt, vilket också syns i figuren nedan. Under andra kvartalet 2020 minskade aktiviteten i den svenska ekonomin med nästan 8 procent jämfört med samma kvartal året innan, vilket kan tillskrivas Corona-pandemin. Återhämtningen gick dock snabbt och redan i efterföljande kvartal så ökade BNP med nästan lika mycket som minskningen i kvartal två. Därefter följde ett par år när läget i Sveriges ekonomi kan beskrivas som gott, innan aktiviteten i ekonomin återigen minskade i kvartal ett, år 2022. Därefter har läget varit relativt skakigt, med omväxlande positiva och negativa förändringar i BNP. I `r BNP_df %>% filter(ar>"2015") %>% filter(användning =="- BNP till marknadspris") %>% filter(ar_kvartal==last(ar_kvartal)) %>% .$kvartal` `r BNP_df %>% filter(ar>"2015") %>% filter(användning =="- BNP till marknadspris") %>% filter(ar_kvartal==last(ar_kvartal)) %>% .$ar` var förändringen i BNP `r gsub("\\.",",",BNP_df %>% filter(ar>"2015") %>% filter(användning =="- BNP till marknadspris") %>% filter(ar_kvartal==last(ar_kvartal)) %>% .$Sasongsransad_forandring)` procent. Aktiviteten ökade således i Sveriges ekonomi under det tredje kvartalet 2024, jämfört med motsvarande kvartal året innan. För en diskussion runt vad som påverkade BNP under `r BNP_df %>% filter(ar>"2015") %>% filter(användning =="- BNP till marknadspris") %>% filter(ar_kvartal==last(ar_kvartal)) %>% .$kvartal` `r BNP_df %>% filter(ar>"2015") %>% filter(användning =="- BNP till marknadspris") %>% filter(ar_kvartal==last(ar_kvartal)) %>% .$ar`, se [SCB](https://www.scb.se/hitta-statistik/statistik-efter-amne/nationalrakenskaper/nationalrakenskaper/nationalrakenskaper-kvartals-och-arsberakningar/pong/statistiknyhet/nationalrakenskaper-3e-kvartalet-2024/){target="_blank"}.
...
### Nybyggnation {-}
Precis som för byggloven, varierar byggandet av småhus mellan kvartalen, men en långsiktig trend var att byggandet minskade tydligt från toppen i  `r Husbyggande_df %>%filter(ar>"2019",hustyp=="småhus") %>% filter(varde==max(varde)) %>% select(kvartal,ar) %>% .[1,]`, då `r Husbyggande_df %>%filter(ar>"2019",hustyp=="småhus") %>% filter(varde==max(varde)) %>% .$varde %>% .[1]` småhus påbörjades, till botten under första kvartalet 2024, då blott 17 småhus påbörjades. Därefter har byggandet börjat öka och i `r Husbyggande_df %>%filter(hustyp=="småhus") %>% filter(ar_kvartal==last(ar_kvartal)) %>% select(kvartal,ar)`, påbörjades `r Husbyggande_df %>%filter(hustyp=="småhus") %>% filter(ar_kvartal==last(ar_kvartal)) %>% .$varde` småhus, vilket är fler än kvartalen innan. För flerbostadshus är trenden relativt likvärdig, men variationen är större från kvartal till kvartal. I `r Husbyggande_df %>%filter(hustyp=="flerbostadshus") %>% filter(ar_kvartal==last(ar_kvartal)) %>% select(kvartal,ar)`, påbörjades `r Husbyggande_df %>%filter(hustyp=="flerbostadshus") %>% filter(ar_kvartal==last(ar_kvartal)) %>% .$varde` lägenheter, vilket är en förbättring jämfört med kvartalet innan.
...
Under första halvåret 2024 steg priserna på småhus tydligt i Dalarna, innan de återigen började sjunka i augusti. I `r priser_df %>% filter(Period==last(Period),region=="Dalarna") %>% select(manad_long,ar)` var priset på ett småhus knappt `r gsub("\\.",",",(round(priser_df %>% filter(Period==last(Period),region=="Dalarna") %>% select(Medelpris)/1000,1)))` miljoner kronor (i genomsnitt), vilket är en ökning jämfört med månaderna innan. 
...
`r storsta_bransch` var den bransch i Dalarna som hade flest anställda som berördes av konkurser i `r konk_bransch_manad` `r konk_bransch_ar`, totalt `r storsta_bransch_antal` personer. Även inom "`r nast_storsta_bransch`" berördes relativt många anställda av konkurser (`r nast_storsta_bransch_antal` personer).

```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.height=5, fig.width=8, fig.align='center'}
gg_konkurser_bransch$anstalla_berorda_konkurser_bransch_Dalarna
...
På sistone har det blivit tydligt att den långsiktiga trenden med stadigt minskande arbetslöshet har brutits. Under 2024
...
Antalet personer som får bistånd varierar en del mellan månaderna, men under 2024 har trenden varit nedåtgående då färre får bistånd jämfört med föregående år. I `r last(ekonomiskt_bistand_SCB_df$månad_år)` var `r last(ekonomiskt_bistand_SCB_df$antal)` personer i behov av sådant stöd, vilket dock är något fler än månaderna innan.
