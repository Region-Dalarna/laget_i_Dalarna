...
### Nybyggnation {-}
Precis som för byggloven, varierar byggandet av småhus mellan kvartalen, men en tydlig trend är att byggandet har börjat minska på senare tid. Sedan toppen i  `r Husbyggande_df %>%filter(ar>"2019",hustyp=="småhus") %>% filter(varde==max(varde)) %>% select(kvartal,ar) %>% .[1,]`, då `r Husbyggande_df %>%filter(ar>"2019",hustyp=="småhus") %>% filter(varde==max(varde)) %>% .$varde %>% .[1]` småhus påbörjades, har antalet minskat och i `r Husbyggande_df %>%filter(hustyp=="småhus") %>% filter(ar_kvartal==last(ar_kvartal)) %>% select(kvartal,ar)`, påbörjades `r Husbyggande_df %>%filter(hustyp=="småhus") %>% filter(ar_kvartal==last(ar_kvartal)) %>% .$varde` småhus. För flerbostadshus är trenden relativt likvärdig, men variationen är större från kvartal till kvartal. I `r Husbyggande_df %>%filter(hustyp=="flerbostadshus") %>% filter(ar_kvartal==last(ar_kvartal)) %>% select(kvartal,ar)`, påbörjades `r Husbyggande_df %>%filter(hustyp=="flerbostadshus") %>% filter(ar_kvartal==last(ar_kvartal)) %>% .$varde` lägenheter.
...
På sistone har priserna återigen börjat stiga och var i `r priser_df %>% filter(Period==last(Period),region=="Dalarna") %>% select(manad_long,ar)`, drygt `r gsub("\\.",",",(round(priser_df %>% filter(Period==last(Period),region=="Dalarna") %>% select(Medelpris)/1000,1)))` miljoner kronor i Dalarna (i genomsnitt). 
...
På sistone har det blivit tydligt att den långsiktiga trenden med stadigt minskande arbetslöshet har brutits. Under första halvåret 2024
