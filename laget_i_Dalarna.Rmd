...
<ul>
<li> Inflationen har gått ned och hushållen och företagens syn på ekonomin har förbättrats på sistone, men Sveriges ekonomi har krymt tre kvartal i rad. </li>
<li> Bostadsmarknaden i Dalarna har upplevt en nedgång i både bygglov, nybyggnation och bostadspriser sedan 2022, men bostadspriserna har återigen börjat öka de senaste månaderna. </li>
<li> Antalet anställda berörda av konkurser i Dalarna har ökat i början av 2024, men samtidigt har antalet avregistrerade företag minskat något. </li>
<li> Arbetslösheten i Dalarna, som är under genomsnittet i Sverige, har efter en lång period av nedgång börjat öka något i slutet av 2023.</li>
...
### Generellt {-}
I `r last(barometern_df %>% filter(Indikator=="Barometerindikatorn")) %>% .$manad_long` `r last(barometern_df %>% filter(Indikator=="Barometerindikatorn")) %>% .$ar` var barometerindikatorn `r gsub("\\.",",",last(barometern_df %>% filter(Indikator=="Barometerindikatorn")) %>% .$varde)`, vilket är en tydlig förbättring jämfört med månaden innan. Företagens och hushållens syn på sin ekonomi verkar således ha förbättrats i Sverige på sistone, även om läget fortfarande kan beskrivas som svagare än normalt. Att barometerindikatorn länge haft ett lågt värde kan delvis förklaras av Rysslands invasion av Ukraina och de effekter denna haft på bland annat tillgången på råvaror. Historiskt varierar barometerindikatorn med konjunkturen och större kriser i samhället, såsom finanskrisen 2008/2009 och Coronapandemin 2020/2021, syns tydligt i diagrammet.
...
#                               skriv_till_diagramfil = spara_figur)
#     
#        test = konj_bransch+geom_line_interactive(aes(tooltip = Indikator,data_id = Indikator))
#    
#    ut = girafe(ggobj = test)
...
Sedan Riksbankens inflationsmål infördes 1993 har inflationen, mätt som årsförändring i KPIF, legat klart under inflationsmålet på 2 procent under långa perioder. Från sin bottennivå i april 2020 ökade dock inflationen väldigt mycket och var som högst i `r KPI_df %>%filter(Period>"1993-01") %>% filter(KPIF==max(KPIF,na.rm=TRUE)) %>% select(manad_long,ar)`, då den i Sverige uppgick till `r gsub("\\.",",",KPI_df %>%filter(Period>"1993-01") %>% filter(KPIF==max(KPIF,na.rm=TRUE)) %>% .$KPIF)` procent. Därefter har inflationstakten sjunkit avsevärt och var i `r KPI_df %>% filter(månad==last(månad)) %>% select(manad_long,ar)`, `r gsub("\\.",",",KPI_df %>% filter(månad==last(månad)) %>% .$KPIF)` procent, vilket bara är något över Riksbankens inflationsmål. En förklaring till att inflationen gick ned i mars 2024 är enligt SCB att priserna på mat sjönk jämfört med månaden innan ([SCB](https://www.scb.se/hitta-statistik/statistik-efter-amne/priser-och-konsumtion/konsumentprisindex/konsumentprisindex-kpi/pong/statistiknyhet/konsumentprisindex-kpi-mars-2024/){target="_blank"}). Sammantaget medför detta att många bedömare tror att Riksbanken sänker sin stryrränta i maj, något som påverkar räntorna på bland annat hushållens bostadslån.
...
På sistone har priserna återigen börjat stiga och var i `r priser_df %>% filter(Period==last(Period),region=="Dalarna") %>% select(manad_long,ar)` strax över `r gsub("\\.",",",(round(priser_df %>% filter(Period==last(Period),region=="Dalarna") %>% select(Medelpris)/1000,1)))` miljoner kronor i Dalarna. 
...
```
Den bransch som drabbades värst av konkurser i `r last(konkurser_df$manad_long)` `r unique(konkurser_branscher_df$år)` var "`r konkurser_branscher_df %>% filter(varde==max(varde)) %>% .$bransch`", där `r konkurser_branscher_df %>% filter(varde==max(varde)) %>% .$varde` personer berördes av konkurser. Även inom hotell och restaurang drabbades relativt många av konkurser.
...
Det finns vissa indikationer på att arbetslösheten har börjat stiga svagt igen, efter att ha minskat stadigt under en lång period.  Arbetslösheten tenderar dock att följa ett visst säsongsmönster, vilket gör det svårt att bedöma om den högre arbetslösheten de senaste månaderna (jämfört med månaderna innan) är en del av en längre trend, eller en följd av säsongsvariationer. Tydligt är i alla fall att långsiktiga trenden sedan mitten av 2021, med stadigt minskande arbetslöshet, har brutits. Arbetslösheten i Dalarna var i januari 2024 på ungefär samma nivå som i januari föregående år. 
...
På sistone har antalet hushåll i behov av ekonomiskt bistånd börjat öka. I `r last(ekonomiskt_bistand_df$Månad)` `r last(ekonomiskt_bistand_df$År)` var `r last(ekonomiskt_bistand_df$antal_hushall)` hushåll i behov av sådant stöd, vilket är ungefär 140 fler än vid den lägsta nivån (oktober 2023).
