...
<ul>
<li> Inflationen är lägre än Riksbankens inflationsmål och hushållen och företagens syn på ekonomin har förbättrats på sistone, samtidigt som aktiviteten i ekonomin (BNP) ökade under tredje kvartalet 2024. </li>
...
<li> Antalet anställda berörda av konkurser i Dalarna har minskat de senaste månaderna, jämfört med samma period året innan. </li>
...
Under första halvåret 2024 steg priserna på småhus tydligt i Dalarna, innan de återigen började sjunka i augusti. I `r priser_df %>% filter(Period==last(Period),region=="Dalarna") %>% select(manad_long,ar)` var priset på ett småhus knappt `r gsub("\\.",",",(round(priser_df %>% filter(Period==last(Period),region=="Dalarna") %>% select(Medelpris)/1000,1)))` miljoner kronor i Dalarna (i genomsnitt), vilket är en ökning jämfört med månaderna innan. 
...
<span class = "kommentar">Tidigare avsnitt</span>
...
Mot slutet av 2023 började antal hushåll i behov av ekonomiskt bistånd att öka igen, innan antalet återigen sjönk under april och maj 2024. I `r last(ekonomiskt_bistand_df$Månad)` `r last(ekonomiskt_bistand_df$År)` var `r last(ekonomiskt_bistand_df$antal_hushall)` hushåll i behov av sådant stöd, vilket är  `r abs(last(ekonomiskt_bistand_df$antal_hushall) - last(ekonomiskt_bistand_df %>% filter(ar_manad!=last(ar_manad)))$antal_hushall)` `r ifelse((last(ekonomiskt_bistand_df$antal_hushall) - last(ekonomiskt_bistand_df %>% filter(ar_manad!=last(ar_manad)))$antal_hushall)<0,"färre", "fler")` än månaden innan.

```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.height=5, fig.width=8, fig.align='center'}
gg_ek_bistand$ekonomiskt_bistand
```

<span class = "kommentar">Nytt avsnitt</span>
