...
Hur läget i ekonomin uppfattas varierar beroende på om det är företagen eller hushållen som tillfrågas. Hushållens syn på ekonomin försämrades kraftigt i `r last(barometern_df$manad_long)` `r last(barometern_df$ar)`, jämfört med månaderna innan, med en barometerindikator på `r gsub("\\.",",",last(barometern_df$varde[barometern_df$Indikator=="Konfidensindikator hushåll"]))` i  `r last(barometern_df$manad_long)` `r last(barometern_df$ar)`, den lägsta siffran sedan april 2024. Hushållens syn på ekonomin visar nu på ett betydligt svagare stämningsläge än normalt. Försämringen hänger sannolikt delvis samman med den osäkra situationen i USA, med risk för ett framtida tullkrig. 
