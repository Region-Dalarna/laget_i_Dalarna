...
# barometern_df[substr(barometern_df$Period,6,7)==last(substr(barometern_df$Period,6,7)),]
  ...
  diagramtitel <- paste0("Barometerindikatorns utveckling mellan ",first(barometern_df$manad_long)," ",first(barometern_df$ar)," och ",last(barometern_df$manad_long)," ",last(barometern_df$ar))
  diagramtitel = str_wrap(diagramtitel,30)
...
```{r, include = FALSE}
  ...
  diagramtitel <- paste0("Arbetslöshet i åldersgruppen 20-64 i ",unique(arbetsmarknadsstatus_tidsserie %>% filter(ar == max(ar)) %>% filter(manad_long==last(manad_long)) %>%  .$manad_long)," ",unique(arbetsmarknadsstatus_tidsserie %>% filter(ar == max(ar)) %>% filter(manad_long==max(manad_long)) %>%  .$ar))
