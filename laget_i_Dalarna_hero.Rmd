...
########### Data som inte uppdateras automagiskt

# Källa: https://sdb.socialstyrelsen.se/if_ekb_manad/resultat.aspx
# Uppdateras genom att hämta en ny version av data
ekonomiskt_bistand_df <-  read.xlsx(here("Data","Socialstyrelsen_2023_11_07.xlsx"),startRow = 2) %>% 
...
nystartade_df <-  read.xlsx(here("Data","Tillvaxtananalys_2023_10_09.xlsx"),startRow = 3)

# Källa: https://foretagsinfo.bolagsverket.se/sok-foretagsinformation-web/statistik
antal_avregistreringar_df<- read.csv("C:/Users/frkjon/Projekt/laget_i_Dalarna/Data/Statistik.csv",encoding="UTF-8") %>% 
...
Priserna på småhus ökade stadigt under en lång period, såväl i Dalarna som i Sverige. Efter toppen i `r priser_df %>% filter(Medelpris==(max(priser_df %>% filter(ar>2009,region%in%c("Dalarna")) %>% select(Medelpris)))) %>% select(manad_long,ar)`, har dock det genomsnittliga priset på småhus i Dalarna minskat med ungefär `r abs((round((priser_df %>% filter(Period==last(Period),region=="Dalarna") %>% select(Medelpris))/(priser_df %>% filter(Medelpris==(max(priser_df %>% filter(ar>2009,region%in%c("Dalarna")) %>% select(Medelpris)))) %>% select(Medelpris)),2)-1)*100)` procent. Minskningen kan delvis förklaras av Rysslands invasion av Ukraina, vilken bland annat medfört en kraftigt ökad inflation med efterföljande räntehöjningar, något som påverkar efterfrågan på bostäder. 
...
<div title="Källa: Arbetsförmedlingen">Varsel innebär att en arbetsgivare förvarnar om en större mängd uppsägningar. Varsel och uppsägning förekommer ofta vid arbetsbrist, men behöver inte leda till att den varslade blir uppsagd. Arbetsgivaren är skyldig att varsla om minst fem personer i samma län riskerar att bli uppsagda på grund av arbetsbrist. Notera även att data för varsel gäller för det län där företaget har sitt huvudkontor. Om ett företag med huvudkontor i Dalarna varslar i andra län räknas dessa varsel under Dalarna. Detta gäller även omvänt: om ett företag varslar anställda om uppsägning i Dalarna men har sitt huvudkontor i ett annat län syns dessa inte i statistiken nedan.</div> 
 
</details>

Antalet varsel i Dalarna på månadsbasis varierar mycket över tid. Ofta varslas inga anställda alls, men ibland berörs 100-tals personer under en enskild månad. På senare år lades flest varsel i `r varsel_manad_df %>% filter(ar>2019) %>% filter(antal==max(antal)) %>% .$manad_ar_long`, då `r varsel_manad_df %>% filter(ar>2019) %>% filter(antal==max(antal)) %>% .$antal` personer berördes. Detta hängde samman med pandemin kopplad till Covid-19, som startade i början av 2020. Som jämförelse varslades `r varsel_manad_df %>% filter(ar>2019) %>% filter(antal==last(antal)) %>% .$antal` personer i `r varsel_manad_df %>% filter(ar>2019) %>% filter(antal==last(antal)) %>% .$manad_ar_long`. Hittills i år har `r varsel_manad_df %>% filter(ar==last(ar)) %>% group_by(region,ar) %>% .$antal %>% sum()` personer varslats om uppsägning i Dalarna, vilket är fler än de  `r varsel_manad_df %>% filter(ar==(as.numeric(last(ar))-1),manad%in%unique(varsel_manad_df %>% filter(ar==last(ar)) %>% .$manad)) %>% group_by(region,ar) %>% .$antal %>% sum()` personer som varslades om uppsägning under motsvarande period föregående år. Den bransch där flest drabbades av varsel i `r tolower(unique(varsel_bransch_df$Månad))` `r unique(varsel_bransch_df$År)` var "`r varsel_bransch_df %>% filter(Antal_varslade==max(Antal_varslade)) %>% .$Näringsgren`", där `r varsel_bransch_df %>% filter(Antal_varslade==max(Antal_varslade)) %>% .$Antal` personer varslades om uppsägning.
...
#           filter(antal>0) 
  ...
  diagram_titel <- paste0("Antal personer berörda av varsel i Dalarna i",tolower(unique(varsel_bransch_df$Månad))," ",unique(varsel_bransch_df$År))
...
```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.height=5, fig.width=8, fig.align='center',include = TRUE}
varsel_branch_manad
...
```{r, include = FALSE}
    ...
    avreg_manad <- SkapaStapelDiagram(skickad_df = antal_avregistreringar_df,
